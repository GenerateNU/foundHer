...
from .repositories import EmployerAnswersRepo
...
async def update_employer_answer(answer_request: EmployerAnswerUpdate, db: Session=Depends(get_db)):
    json_compatible_item_data = await EmployerAnswersRepo.update(db=db, employer_answer_data=answer_request, id=answer_request.id)
