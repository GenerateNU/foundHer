...
class EmployerAnswersRepo:
 ...
 async def create(db: Session, employer_answer: schemas.EmployerAnswerCreate):
        db_employer_answer = models.Employer_Answer(user_id=employer_answer.user_id,question_id=employer_answer.question_id,answers=employer_answer.answers)
 ...
 def fetch_by_id(db: Session,_id):
     return db.query(models.Employer_Answer).filter(models.Employer_Answer.id == _id).first()
 
 def fetch_all(db: Session, skip: int = 0, limit: int = 100):
     return db.query(models.Employer_Answer).offset(skip).limit(limit).all()
 
 async def delete(db: Session,employer_answer_id):
     db_employer_answer= db.query(models.Item).filter_by(id=employer_answer_id).first()
